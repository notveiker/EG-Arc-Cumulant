...
Full addresses, tx hashes, and the `cast`-based note-seed step are in
[`contracts/DEPLOYMENT.md`](contracts/DEPLOYMENT.md). (Arc's USDC routes `transferFrom` through a
native precompile that forge's local script EVM can't execute, so `make deploy-arc` deploys +
seeds markets/basket/tranche via forge and creates the note via `cast`.)
