...
// Persistence layer (Supabase) — ported 1:1 from Cumulant. Two differences vs Arc:
//   1. Every helper tolerates a null client (supabase unconfigured): it
//      early-returns a safe empty value ([] / null / false / zeroed stats) and
//      records nothing, so the backend boots and serves on-chain + Polymarket
//      data without a database.
//   2. The `tx_signature` / `onchain_tx_signature` / `redemption_tx_signature`
//      columns store EVM transaction HASHES (0x… 66 chars on Arc). The COLUMN
//      NAMES are kept identical for compatibility so the schema migrations and
//      route/service callers are unchanged.
...
export async function getBundleWithLegs(
  id: string,
): Promise<BundleWithLegs | null> {
  ...
  const nav = legs.reduce((sum, leg) => {
    const value =
      leg.status === "active" ? leg.probability : (leg.resolution_value ?? 0);
...
export async function getPositionsByBundle(
  bundleId: string,
): Promise<Position[]> {
  if (!supabase) return [];
  const { data, error } = await supabase
...
/**
 * Look up a transaction by its on-chain tx hash (for idempotent confirms).
 * On Arc this is an EVM transaction hash; the column name `tx_signature` is kept
 * for compatibility with the Cumulant schema.
 */
...
export async function createPPNVault(
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    console.warn(
      "createPPNVault: retrying without columns",
      Array.from(knownOptional),
    );
  }
...
export async function getPPNVaultsByWallet(
  walletAddress: string,
): Promise<PPNVault[]> {
  if (!supabase) return [];
  ...
  //     with a null hash. `/confirm` only stamps the hash after the Arc write
  //     confirms, so this filter is the dividing line between "real principal
  //     on-chain" and "abandoned row".
  //   - `status != 'withdrawn'` — `/onchain/redeem/confirm` flips status to
  //     `withdrawn` but leaves the original deposit hash in place, so without
  //     this gate a redeemed vault would keep showing up on the portfolio at
  //     full principal even though the user already has the USDC back in their
  //     wallet.
  const { data, error } = await supabase
    .from("ppn_vaults")
    .select("*")
 ...
 * `tranche_kind` is null, AND excludes rows with no on-chain hash (rows from
 * cancelled transactions). Used by the tranche portfolio UI.
 */
export async function getTranchesByWallet(
  walletAddress: string,
): Promise<PPNVault[]> {
  if (!supabase) return [];
  const { data, error } = await supabase
...
/**
 * ALL note/tranche vaults for a wallet — no status or hash filter — so the
 * transaction-ledger enrichment can tag each tx with its product type by
 * matching the tx hash against the vault's deposit/redemption hash (withdrawn
 * vaults still hold the redemption hash, so they must be kept).
 */
export async function getPPNVaultsForLedger(
  walletAddress: string,
): Promise<PPNVault[]> {
  if (!supabase) return [];
  const { data, error } = await supabase
...
/**
 * Update the on-chain artifacts for a PPN vault after an Arc write confirms.
 */
...
export async function createPriceAlert(
  alert: Omit<
    PriceAlert,
    "id" | "created_at" | "triggered" | "triggered_at" | "triggered_nav"
  >,
...
export async function getActiveAlertsByBundle(
  bundleId: string,
): Promise<PriceAlert[]> {
  if (!supabase) return [];
  const { data, error } = await supabase
