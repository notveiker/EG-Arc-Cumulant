...
// Input validation for the Cumulant routes — ported from Cumulant with the only
// change being the on-chain identity format: wallet_address is now an EVM
// (0x + 40 hex) address rather than a Arc address, and tx hashes are EVM hashes
// (0x + 64 hex). The financial fields are unchanged.
...
export const createBundleSchema = z
  .object({
    ...
    resolution_date: z
      .string()
      .datetime({ message: "resolution_date must be ISO date format" }),
...
export const evmAddress = z
  .string()
  .regex(/^0x[0-9a-fA-F]{40}$/, "wallet_address must be a 0x-prefixed EVM address");

// An EVM transaction hash (0x + 64 hex, 66 chars total).
export const txHash = z
  .string()
  .regex(/^0x[0-9a-fA-F]{64}$/, "signature must be a 0x-prefixed EVM tx hash");
