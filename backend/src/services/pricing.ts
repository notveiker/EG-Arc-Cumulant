import { Leg, NAVResult } from '../types.js';
import { calculateNAV, calculateIssuePrice } from './nav.js';
import { getMarketProbability, fetchMarketByConditionId } from './polymarket.js';
import {
  getLegsByBundleId,
...
// The on-chain adapter provides the
// single EVM adapter `services/onchain.js`. The vault-price read uses the EVM
// adapter's `getVaultPrice` (aliased to avoid colliding with the cached export
// below); resolution mirroring + bundle finalization stay 1:1.
import {
  finalizeBundleIfReady,
  resolveLegOnchainMirror,
  getVaultPrice as getVaultPriceOnchain,
} from './onchain.js';
...
export async function getVaultPrice(
  ...
  try {
    const vault = await getVaultPriceOnchain(bundleId);
    if (!vault) return null;
    const entry: VaultPriceEntry = {
      issue_price: vault.issue_price,
...
export async function checkAndUpdateResolutions(bundleId: string): Promise<Leg[]> {
  ...
  await Promise.allSettled(
    activeLegs.map(async (leg) => {
      ...
      if (resolved) {
        ...
        // Mirror the resolution on-chain (Arc). Failures are logged but do not
  ...
  // If this resolution round completed the bundle, finalize the bundle on Arc.
