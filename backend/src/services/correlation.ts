...
// ESM shim — this module was ported from a CommonJS codebase that used __dirname.
const __dirname = path.dirname(fileURLToPath(import.meta.url));

/**
 * Cumulant correlation service.
 *
 * Consumes the audited ML artifacts in `cumulant-correlation-models/` and
 ...
 *      per-leg weights that get written into the on-chain BasketVault.
...
const CANDIDATE_DIRS = [
  path.resolve(__dirname, '..', '..', '..', 'cumulant-correlation-models'),
  path.resolve(__dirname, '..', '..', 'cumulant-correlation-models'),
  path.resolve(process.cwd(), '..', 'cumulant-correlation-models'),
  path.resolve(process.cwd(), 'cumulant-correlation-models'),
];
...
export function optimizeWeights(
  ...
  // Raw weight = (1 - avgCorr) / stdDev (Inverse-Variance / risk-adjusted weighting)
  // stdDev of a binary option outcome is sqrt(p * (1 - p)). We clamp variance to [0.04, 0.25]
  // (which maps to p in [0.05, 0.95]) to prevent division by zero or extreme concentration.
  let raw = avgCorr.map((c, i) => {
    const p = legs[i].probability ?? 0.5;
    const variance = Math.max(0.04, p * (1 - p));
    const stdDev = Math.sqrt(variance);
    return Math.max(0.01, (1 - c) / stdDev);
  });
