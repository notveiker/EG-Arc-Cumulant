// Distribution Markets discovery + quoting service.
//
// Chain-agnostic: discovers live Polymarket events whose mutually-exclusive
// outcome sets approximate a probability distribution, scores them for launch
// readiness, and quotes a discrete-L2 distribution-AMM trade settled in USDC on
// Circle Arc. Pricing is sourced from the Polymarket Gamma + CLOB feeds; nothing
// here is hardcoded — candidates and depth are refreshed live.

import { fetchEvents, fetchMarkets } from './polymarket.js';
import { proxiedFetch } from './proxy.js';
import { assessQuality, classifyCategory, type Category } from './nlp.js';
import { type PolymarketEvent, type PolymarketMarket } from '../types.js';
...
function normalize(values: number[]): number[] {
  // Coerce non-finite weights to 0 so a NaN/Infinity input can't poison the whole curve.
  const clean = values.map((n) => (Number.isFinite(n) ? Math.max(0, n) : 0));
  const total = clean.reduce((acc, n) => acc + n, 0);
  if (!(total > 0)) return clean.map(() => 1 / Math.max(1, clean.length));
...
export async function discoverDistributionCandidates(params: {
  ...
  try {
    [events, markets] = await Promise.all([
      fetchEvents({ limit: 250, active: true }),
      fetchMarkets({ limit: 1200, active: true, closed: false }),
    ]);
  } catch (err) {
    console.error('[distribution] upstream Polymarket fetch failed', err);
  }

  // If Polymarket is unreachable, serve the last good candidate set rather than 500ing; if there's
  // no warm cache yet, return an empty-but-valid result so the UI shows "no markets" cleanly.
  if (events.length === 0 && markets.length === 0) {
    ...
    return {
      ...
      funnel: {
        ...
        input_markets: 0,
        kept_candidates: 0,
        rejected: { too_few_bands: 0, low_volume: 0, low_depth: 0, low_quality: 0, low_distribution_fit: 0 },
        filters,
      },
      fetched_at: new Date().toISOString(),
    };
...
/** Input error carrying the HTTP status the route should surface (vs a 500 server fault). */
...
export async function quoteDistributionCandidate(args: {
  ...
  if (!args.candidateId) throw new DistributionInputError(400, 'candidate_id is required');
  ...
  if (!candidate) throw new DistributionInputError(404, `Unknown distribution candidate: ${args.candidateId}`);
  if (args.weights.length !== candidate.bands.length) {
    throw new DistributionInputError(400, `Expected ${candidate.bands.length} curve weights`);
  }
  const collateral = Number(args.collateralUsdc);
  if (!Number.isFinite(collateral) || collateral <= 0) {
    throw new DistributionInputError(400, 'collateral_usdc must be positive');
  }
...
export async function buildDistributionLaunchPlan(candidateId: string): Promise<DistributionLaunchPlan> {
  if (!candidateId) throw new DistributionInputError(400, 'candidate_id is required');
  const { candidates } = await discoverDistributionCandidates({ limit: 40 });
  const candidate = candidates.find((item) => item.id === candidateId);
  if (!candidate) throw new DistributionInputError(404, `Unknown distribution candidate: ${candidateId}`);
