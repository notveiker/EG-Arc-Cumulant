...
function treasuryAddress(): string {
  const addr = resolverAddress() ?? config.resolver ?? null;
  if (!addr) throw new Error('Protocol treasury (resolver) is not configured.');
  return addr;
}
...
export async function prepareContinuousOpen(args: {
  ...
  // required collateral; the frontend signs `USDC.transfer(treasury, collateral)`
  // via wagmi and posts the resulting tx hash to /continuous/open/confirm, which
  // verifies the receipt before recording the position.
  return {
    prepared_tx: '',
    sender: args.owner,
...
export async function settleContinuousPosition(args: { owner: string; positionId: string }): Promise<SettleResult> {
  ...
  pos.settled = true;
  pos.settle_tx_hash = settleTxHash ?? undefined;
...
export async function closeContinuousPosition(args: { owner: string; positionId: string }): Promise<CloseResult> {
  ...
  pos.settled = true;
  pos.settle_tx_hash = closeTxHash ?? undefined;
