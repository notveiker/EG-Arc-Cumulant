...
export default function BasketDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL ?? "http://localhost:13201"}/api/bundles`)
      .then((r) => r.json())
      .then((bundles: Array<{ id: string; name: string }>) => {
        const match = bundles.find((b) => b.name === id);
        if (!match) return;
        return fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL ?? "http://localhost:13201"}/api/deposit/vault-price/${match.id}`
        ).then((r) => r.json());
      })
  ...
  return (
    <>
      ...
      <PageFrame>
        ...
        <style dangerouslySetInnerHTML={{ __html: BASKET_DETAIL_CSS }} />
      </PageFrame>
    </>
  );
}
...
function BasketBuyPanel({
  ...
  // price; buy side uses vaultIssuePrice so the quote matches what the contract mints.
  ...
  // matches what the Arc write returns. Falls
  ...
  // receives after fees. The Arc redeem path closes the local position;
  // `sellQty` is still
  ...
  const onchainHeldQty =
    resolvedBundleUuid
      ? pbuBalances.balances.find((b) => b.bundleId === resolvedBundleUuid)
          ?.uiAmount ?? 0
      : 0;
  const virtualHeldQty = groupVirtualByUiBundle(activeAddress)
  ...
  // Arc settlement only applies the flat early-exit fee (`EARLY_EXIT_FEE_BPS`,
  ...
  // ---- Submit -------------------------------------------------------
  //
  // Both Buy and Sell run through the Arc deposit-client helpers. The
  // sandbox dispatch fires only after the Arc write confirms, so the portfolio card
