...
export default function PpnPage() {
  ...
  // Index the wallet's open notes from the Arc-backed backend so the Positions
    ...
    try {
      for (const id of vaultIds) {
        if (action === "withdraw") {
          ...
          await ppnRedeem({ wallet, vaultId: id, signNote: signRedeem });
        } else if (action === "divest") {
           
          await ppnDivest({ wallet, vaultId: id, signNote: signRedeem });
        } else {
           
          await ppnCloseEarly({ wallet, vaultId: id, signNote: signRedeem });
        }
      }
      await finishExitFlow(rowKey);
    } catch (err) {
  ...
  return (
    <>
      ...
      <PageFrame wide zoom={0.8}>
        <style dangerouslySetInnerHTML={{ __html: PPN_CSS }} />
