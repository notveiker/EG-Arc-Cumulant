 ...
 * Both carry their real on-chain tx hash so the Tx link resolves.
 */
function distRowsForPosition(p: ContinuousPosition): TxRow[] {
  ...
  const out: TxRow[] = [
    {
      ...
      tokens: null,
      fee_usdc: 0,
      tx_signature: p.open_tx_hash ?? null,
  ...
  if (p.settled) {
    ...
    out.push({
      ...
      tokens: null,
      fee_usdc: 0,
      tx_signature: p.settle_tx_hash ?? null,
  ...
  const fetchRows = useCallback(
    async (opts: { background?: boolean } = {}): Promise<void> => {
      ...
      try {
        ...
        // tx hashes). Best-effort: a distribution fetch failure never blocks the
        // core ledger.
        let distRows: TxRow[] = [];
        try {
