...
export default function PortfolioPage() {
  ...
  const virtualGroupsForWallet: GroupedVirtualPosition[] =
    walletReady && appWalletAddress
      ...
      : [];
  const virtualTokensByUuid = virtualGroupsForWallet.reduce<Record<string, number>>(
    (acc, g) => {
  ...
  // rows never get here. lend/loan are pure reducer (no on-chain
  // position behind them) and unaffected by this pass.
  //
    ...
    <>
      ...
      <PageFrame>
        ...
        {(() => {
            ...
            return (
              <div className="portfolio-card" style={{ textAlign: "center", padding: "34px 20px" }}>
                ...
                <div style={{ color: C.textSecondary, fontFamily: FS, fontSize: 13 }}>
                  Portfolio data is pulled from the connected account and the Arc-backed backend indexer.
                </div>
              </div>
            );
          }
          ...
          // USDC cash position. Pulls straight from the Arc balance poll.
          if (liveUsdc > 0) {
            rows.push({
              key: "usdc",
              value: liveUsdc,
