"use client";
/**
 * Portfolio state container for the Cumulant authenticated app shell.
 *
 ...
 * session; real balances will be sourced from the active Arc account once that
 * integration lands.
 *
 * Design:
...
function reducer(state: SandboxState, action: Action): SandboxState {
  switch (action.type) {
    ...
    case "tranche/deposit": {
      if (action.usdcAmount <= 0 || (!IS_LEGACY && action.usdcAmount > state.usdc)) return state;
      ...
      return {
        ...state,
        usdc: IS_LEGACY ? state.usdc : state.usdc - action.usdcAmount,
