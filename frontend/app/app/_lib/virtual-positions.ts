 ...
 *
 * On Arc (EVM) these were never on-chain objects — they live entirely in a
 * simple keyed localStorage store. The `signature` field carries the EVM tx
 * hash that minted the position; `marketId` / `positionId` carry the resolved
 * on-chain bundle id + vault address used at deposit time.
 */
...
const STORAGE_KEY = "cumulant:virtualPositions:v1";
...
/**
 * Remove the virtual position matching a (wallet, marketId, positionId)
 * triple — the resolved on-chain bundle id + vault address recorded at
 * deposit time. (On Arc these are EVM-side ids.)
 */
export function clearVirtualPositionByIds(
  wallet: string,
  marketId: string,
  positionId: string,
): void {
