...
export function useLiveBaskets(): LiveBasketState {
  // Always seed from a CONSTANT "loading" so the server and the client's first
  // (hydration) render produce identical HTML. Seeding from the mutable
  // module-level `cacheState` here let the client hydrate against an already-
  // populated "ok" cache while the server had rendered "loading" — a text
  // hydration mismatch. The effect below immediately syncs to the live cache.
  const [state, setState] = useState<LiveBasketState>({ status: "loading" });
