...
type RawBook = { token_id: string; bids?: BookLevel[]; asks?: BookLevel[] };
...
export async function fetchOrderbooks(
  ...
  // The Cumulant backend wraps payloads in an envelope: { ok, data }.
  // The orderbook list lives at body.data.books. We also tolerate an
  // un-enveloped { books } shape so the parser never silently drops
  // data if the route changes underneath us.
  const body = unwrap(await res.json()) as {
    ok?: boolean;
    data?: { books?: RawBook[] } | RawBook[];
    books?: RawBook[];
  };

  const data = body.data;
  const books: RawBook[] =
    (Array.isArray(data) ? data : data?.books) ?? body.books ?? [];
